...
const FavCatPost = require("../model/FavCatPost");
...
router.get("/", async (req, res) => {
  try {
    // find() -> get all data
    const catposts = await FavCatPost.find();
    if(!catposts){
      res.json({
      status:"400",
...
router.post('/', async (req, res) => {
  const catposts = await new FavCatPost({
    id: req.body.id,
...
router.get("/:catpostId", async (req, res) => {
  try {
    ...
    FavCatPost.findById(req.params.catpostId);
    if(!catposts){
      res.json({
      status:"400",
...
router.patch("/:catpostId", async (req, res) => {
  try {
    ...
    const updateCatPost = await FavCatPost.updateOne(
...
router.delete("/:catpostId", async (req, res) => {
  try {
    ...
    const deleteCatPost = await FavCatPost.deleteOne(
      {_id :req.params.catpostId}
    );
    if(!deleteCatPost){
      res.json({
